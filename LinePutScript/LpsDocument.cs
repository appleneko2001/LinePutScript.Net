...
public class LpsDocument<T> : ILPS, IReadOnlyList<ILine>, IReadOnlyCollection<ILine> where T : IList<ILine>, new()
{
    ...
    {
        get { return FindorAddLine<Line>(lineName); }
    ...
    /// 是否只读
    /// </summary>
    public bool IsReadOnly => Assemblage.IsReadOnly;
    ...
    /// </summary>
    /// <param name="lineName">用于定义匹配的名称</param>
    /// <param name="defaultValue">Default boolean value if the line is not exist.</param>
    /// <returns>如果找到相同名称的line,则为True; 否则为false</returns>
    public bool GetBool(string lineName, bool defaultValue = false) => FindLine(lineName)?.GetBoolean() ?? defaultValue;
    ...
    public int GetInt(string lineName, int defaultValue = default)
    {
        ...
        return line?.InfoToInt ?? defaultValue;
    }
    ...
    {
        var line = FindLine(lineName);
        return line?.GetFloat() ?? defaultValue;
    }
    ...
    {
        var line = FindLine(lineName);
        return line?.GetDateTime() ?? defaultValue;
    }
...
public class LpsDocument : LpsDocument<List<ILine>>
{
    ...
    {
        return LineNode == Assemblage.Count ? default : Assemblage[LineNode++];
    }
