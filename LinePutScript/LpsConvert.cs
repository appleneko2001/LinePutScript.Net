...
public static class LpsConvert
{
    ...
    public static List<TLine> SerializeObjectToList<TLine>(object value, bool fourceToString = false)
    ...
    /// </param>
    /// <returns>LPS</returns>
    public static TLPS SerializeObjectToLPS<TLPS, TLine>(object value, bool fourceToString = false)
        where TLPS : ILPS, new() where TLine : ILine, new()
    {
    ...
    public static TLine SerializeObjectToLine<TLine>(object value, string linename, bool fourceToString = true)
        where TLine : ILine, new()
    {
        var list = SerializeObjectToList<TLine>(value, fourceToString: fourceToString);
    ...
    public static string GetObjectString(object? value, ConvertType type = ConvertType.Default,
        LineAttribute? att = null)
    {
        //如果为null储存空
        if (value == null)
        {
...
#pragma warning restore CS8605 // 取消装箱可能为 null 的值。
    ...
    /// <param name="linename">行名字</param>
    /// <returns></returns>
    public static TLine GetObjectLine<TLine>(object? value, string linename, ConvertType type = ConvertType.Default,
        LineAttribute? att = null) where TLine : ILine, new()
    {
...
#pragma warning disable CS8602 // 解引用可能出现空引用。
    ...
    public static object? GetStringObject(string value, Type type, ConvertType convtype = ConvertType.Default,
        LineAttribute? att = null)
    {
        if (value == "")
        {
        ...
        switch (ct)
        {
            case ConvertType.ToList:
                IList list =
                    (IList)Activator
                        .CreateInstance(type); //type.GetConstructor(Array.Empty<Type>()).Invoke(null, null);
...
#pragma warning restore CS8604 // 引用类型参数可能为 null。
            ...
            case ConvertType.Class:
                ...
                foreach (var mi in type.GetProperties(BindingFlags.Instance | BindingFlags.Public |
    ...
    /// <param name="att">附加参数,若有</param>
    /// <returns>指定Type的Object</returns>
    public static object? GetSubObject(ISub sub, Type type, ConvertType convtype = ConvertType.Default,
        LineAttribute? att = null)
    {
        ConvertType ct = convtype == ConvertType.Default ? GetObjectConvertType(type, att) : convtype;
        if (sub is ILine line)
            switch (ct)
            {
                ...
                case ConvertType.Class:
                    ...
                    foreach (var mi in type.GetFields(BindingFlags.Instance | BindingFlags.Public |
                                                      BindingFlags.NonPublic))
                    {
                        var latt = mi.GetCustomAttribute<LineAttribute>();
                        if (latt != null)
