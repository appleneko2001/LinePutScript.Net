...
public class Line<T> : Sub, ILine, IReadOnlyList<ISub> where T : IList<ISub>, new()
{
    /// <summary>
    /// 创建一行
    /// </summary>
    ...
    public override void Load(string lps)
    {
        ...
        var st = sts[0].Split(new char[1] { '#' }, 2); //第一个
        Name = st[0];
    ...
    public StringStructure Texts
    {
        get
        {
            _texts ??= new StringStructure((x) => RawText = x, () => RawText);
            return _texts;
        }
    }

    private StringStructure? _texts;
    ...
    public ISub[] SearchAll(string value)
    {
        List<ISub> subs = new List<ISub>();
        foreach (ISub su in Subs)
    ...
    {
        long hash = GetHashCode(Name) * 2 + GetHashCode(RawInfo) * 3 + GetHashCode(RawText) * 4;
        foreach (ISub su in Subs)
        {
    ...
    {
        return Subs.Count == 0 ? default : Subs[0];
    }
    ...
    {
        return Subs.Count == 0 ? default : Subs?.Last();
    }
    ...
    public bool GetBool(string subName, bool defaultValue = false)
    {
        ...
        return sub?.InfoToBoolean ?? defaultValue;
    }
    ...
    public int CompareTo(ILine? other)
    {
        if (other == null)
            return int.MaxValue;
        var comp = string
            .Compare(Name, other.Name, StringComparison.InvariantCulture);
        if (comp != 0)
            return comp;
