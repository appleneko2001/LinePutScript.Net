...
public class LPS_D<T> : ILPS where T : IDictionary<string, ILine>, new()
{
    ...
    {
        return !Assemblage.TryGetValue(lineName, out var v) ?
            Array.Empty<ILine>() : new[] { v };
    }
    ...
    public ILine[] FindAllLine(string lineName, string lineinfo)
    {
        ...
        return v == null ? Array.Empty<ILine>() : new[] { v };
    }
    ...
    {
        return Assemblage.TryGetValue(lineName, out var line) ? line : default;
    }
    ...
    {
        var line = FindLine(lineName);
        if (line != null)
            return line;

        line = new Line_D { Name = lineName };
        AddLine(line);
        return line;
    }
    ...
    {
        var lines = new List<ISub>();
        foreach (var li in Assemblage.Values)
        {
    ...
    {
        return Assemblage.Values
            .Select(li => li.Find(subName, subinfo))
            .FirstOrDefault(l => l != null);
    }
    ...
    {
        return Assemblage.Values
            .Where(li => li.Name.Contains(value)).ToArray();
    }
    ...
    public ISub[] SearchAllSub(string value)
    {
        ...
        foreach (var li in Assemblage.Values)
        {
            lines.AddRange(li.SearchAll(value));
        }
    ...
    {
        return Assemblage.Values
            .Select(li => li.Search(value)).FirstOrDefault(l => l != null);
    }
    ...
    /// </summary>
    /// <param name="lps">包含要加载的LPS文档的字符串</param>
    public void Load<TValue>(string lps) where TValue : ILine, new()
    {
    ...
    {
        get => FindOrAddLine(lineName);
        set => AddorReplaceLine(value);
    }
    ...
    {
        return Assemblage.Values.Sum(li => li.GetLongHashCode());
    }
    ...
    public int Length
    {
        get
        {
            ...
            foreach (var li in Assemblage.Values)
            {
                ...
                l += li.Sum(sb => sb.Name.Length + sb.GetStoreString().Length + 3);
            }
    ...
    public bool GetBool(string lineName, bool defaultValue = false) => 
        FindLine(lineName)?.GetBoolean() ?? defaultValue;
    ...
    /// </param>
    public void SetBool(string lineName, bool value) => FindOrAddLine(lineName).SetBoolean(value);
    ...
    public int GetInt(string lineName, int defaultValue = default) => 
        FindLine(lineName)?.InfoToInt ?? defaultValue;
    ...
    public void SetInt(string lineName, int value) =>
        FindOrAddLine(lineName).InfoToInt = value;
    ...
    public long GetInt64(string lineName, long defaultValue = default) => 
        FindLine(lineName)?.InfoToInt64 ?? defaultValue;
    ...
    public void SetInt64(string lineName, long value) =>
        FindOrAddLine(lineName).InfoToInt64 = value;
    ...
    public string? GetString(string lineName, string? defaultValue = default) =>
        FindLine(lineName)?.Info ?? defaultValue;
    ...
    public void SetString(string lineName, string? value) =>
    ...
    public double GetDouble(string lineName, double defaultValue = default) => 
        FindLine(lineName)?.InfoToDouble ?? defaultValue;
    ...
    public void SetDouble(string lineName, double value) =>
        FindOrAddLine(lineName).SetDouble(value);
    ...
    /// 如果没找到,则返回默认值
    /// </returns>
    public float GetFloat(string lineName, float defaultValue = default) => 
        FindLine(lineName)?.GetFloat() ?? defaultValue;
    ...
    public void SetFloat(string lineName, float value) => FindOrAddLine(lineName).SetFloat(value);
    ...
    public DateTime GetDateTime(string lineName, DateTime defaultValue = default) => 
        FindLine(lineName)?.GetDateTime() ?? defaultValue;
    ...
    /// <returns>获取或设置对 double 属性的line</returns>
    public float this[gflt lineName]
    {
    ...
    /// <returns>如果找到相同名称的Line的第一个元素,则为该元素的从零开始的索引; 否则为 -1</returns>
    [Obsolete("错误:字典没有引索")]
    public int IndexOf(ILine line) => throw new ArrayTypeMismatchException();
    ...
    /// <param name="newLine">要添加的Line</param>
    [Obsolete("错误:字典没有引索")]
    public void Insert(int index, ILine newLine) => AddLine(newLine);
